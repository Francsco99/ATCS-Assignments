...
print("Phase 1: Collecting of the explicit brands.")
dict_={'spec':[],
      'title':[], 
      'brand':[]}
files = sorted([join(r,file).replace("monitor/monitor_specs/","").replace(".json","") for r,d,f in walk("monitor/monitor_specs/") for file in f])
...
for item in files:
    f = open("monitor/monitor_specs/"+item+".json")
    data = json.load(f)
    found_brand=False
    title=""
...
"""
print("Phase 2: Brand cleaning- In this step we adopt a series of rules to clean brands and non-products.")
...
for item in to_delete_files_rules:
   with open("monitor/monitor_specs/"+item+".json", 'r') as myfile:
...
output.to_csv("domain_knowledge/output.csv", sep=',',encoding='utf-8', index=False)